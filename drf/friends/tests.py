...
def create_second_user(db):
    serializer = UserSerializer(data={'email': "newuser@example.com", 'password': "newpassword123",
                                      'username': "testuser2"})
    if serializer.is_valid():
        return serializer.save()
...
def test_accept_friend_request(api_client, create_user, create_second_user):
    ...
    # Проверка на то, что при взаимной отправке друг другу запросов,
    # пользователи автоматически добавляются друг другу в друзья
    profile = api_client.get('/accounts/profile/')
